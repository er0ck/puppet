    ...
    with_puppet_running_on(master,{}) do
      ...
      agents.each do |agent|
        on(agent, puppet("agent -t --server #{master.hostname} --environment #{tmp_environment}"),
          ...
          #assert_match(/Notice: \d+/, result.stdout, 'first uptime was not as expected')
          assert_match(/"seconds"=>\d+,/, result.stdout, 'first module uptime was not as expected')
          #uptime = result.stdout.match(/Notice: (\d+)/)[0]
          module_uptime = result.stdout.match(/"seconds"=>(\d+),/)[0]
        end
        sleep 1
        on(agent, puppet("agent -t --server #{master.hostname} --environment #{tmp_environment}"),
           :accept_all_exit_codes => true) do |result|
          #assert_match(/Notice: \d+/, result.stdout, 'second uptime was not as expected')
          assert_match(/"seconds"=>\d+,/, result.stdout, 'second module uptime was not as expected')
          #uptime2 = result.stdout.match(/Notice: (\d+)/)[0]
